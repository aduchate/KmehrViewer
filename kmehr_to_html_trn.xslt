...
The purpose of this XSLT is to generate an HTML view from a khmer file respecting GPMigration rules.
Input GPMigration file must be a valid sumehr.
...
GPMigration definition: https://www.ehealth.fgov.be/standards/kmehr/content/website/home/rules/specialmessages/GP_software_migration.xml
    ...
    <xsl:template name="get-translationdictionary-path">
        <xsl:choose>
            <xsl:when test="$translationdictionary = '' ">
                <xsl:value-of select="concat($annexdirectory,'gpmigration_visualization_translation.xml')"/>
            </xsl:when>
            <xsl:otherwise>
